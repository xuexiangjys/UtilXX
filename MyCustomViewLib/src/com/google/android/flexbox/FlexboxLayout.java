...
/**
 * A layout that arranges its children in a way its attributes can be specified
 * like the CSS Flexible Box Layout Module. This class extends the
 * {@link ViewGroup} like other layout classes such as {@link LinearLayout} or
 * {@link RelativeLayout}, the attributes can be specified from a layout XML or
 * from code.
 * 
...
public class FlexboxLayout extends ViewGroup {
	...
	@IntDef({ FLEX_DIRECTION_ROW, FLEX_DIRECTION_ROW_REVERSE, FLEX_DIRECTION_COLUMN, FLEX_DIRECTION_COLUMN_REVERSE })
	...
	/**
	 * The direction children items are placed inside the Flexbox layout, it
	 * determines the direction of the main axis (and the cross axis,
	 * perpendicular to the main axis).
	 * <ul>
	 * <li>
	 * {@link #FLEX_DIRECTION_ROW}: Main axis direction -> horizontal. Main
	 * start to main end -> Left to right (in LTR languages). Cross start to
	 * cross end -> Top to bottom</li>
	 * <li>
	 * {@link #FLEX_DIRECTION_ROW_REVERSE}: Main axis direction -> horizontal.
	 * Main start to main end -> Right to left (in LTR languages). Cross start
	 * to cross end -> Top to bottom.</li>
	 * <li>
	 * {@link #FLEX_DIRECTION_COLUMN}: Main axis direction -> vertical. Main
	 * start to main end -> Top to bottom. Cross start to cross end -> Left to
	 * right (In LTR languages).</li>
	 * <li>
	 * {@link #FLEX_DIRECTION_COLUMN_REVERSE}: Main axis direction -> vertical.
	 * Main start to main end -> Bottom to top. Cross start to cross end -> Left
	 * to right (In LTR languages)</li>
	 * </ul>
	 ...
	 * <li>{@link #FLEX_WRAP_WRAP_REVERSE}: The flex container is multi-line.
	 * The direction of the cross axis is opposed to the direction as the
	 * {@link #FLEX_WRAP_WRAP}</li>
	 * </ul>
	 * The default value is {@link #FLEX_WRAP_NOWRAP}.
	 */
	private int mFlexWrap;

	@IntDef({ JUSTIFY_CONTENT_FLEX_START, JUSTIFY_CONTENT_FLEX_END, JUSTIFY_CONTENT_CENTER, JUSTIFY_CONTENT_SPACE_BETWEEN, JUSTIFY_CONTENT_SPACE_AROUND })
	@Retention(RetentionPolicy.SOURCE)
	...
	/**
	 * This attribute controls the alignment along the main axis. The default
	 * value is {@link #JUSTIFY_CONTENT_FLEX_START}.
	 */
	private int mJustifyContent;

	@IntDef({ ALIGN_ITEMS_FLEX_START, ALIGN_ITEMS_FLEX_END, ALIGN_ITEMS_CENTER, ALIGN_ITEMS_BASELINE, ALIGN_ITEMS_STRETCH })
	@Retention(RetentionPolicy.SOURCE)
	...
	/**
	 * This attribute controls the alignment along the cross axis. The default
	 * value is {@link #ALIGN_ITEMS_STRETCH}.
	 */
	private int mAlignItems;

	@IntDef({ ALIGN_CONTENT_FLEX_START, ALIGN_CONTENT_FLEX_END, ALIGN_CONTENT_CENTER, ALIGN_CONTENT_SPACE_BETWEEN, ALIGN_CONTENT_SPACE_AROUND, ALIGN_CONTENT_STRETCH })
	@Retention(RetentionPolicy.SOURCE)
	...
	/**
	 * This attribute controls the alignment of the flex lines in the flex
	 * container. The default value is {@link #ALIGN_CONTENT_STRETCH}.
	 */
	private int mAlignContent;

	/**
	 * The int definition to be used as the arguments for the
	 * {@link #setShowDivider(int)}, {@link #setShowDividerHorizontal(int)} or
	 * {@link #setShowDividerVertical(int)}. One or more of the values (such as
	 * {@link #SHOW_DIVIDER_BEGINNING} | {@link #SHOW_DIVIDER_MIDDLE}) can be
	 * passed to those set methods.
	 */
	...
	/**
	 * Constant to show a divider at the beginning of the flex lines (or flex
	 * items).
	 */
	...
	/**
	 * Indicates the divider mode for the {@link #mDividerDrawableHorizontal}.
	 * The value needs to be the combination of the value of
	 * {@link #SHOW_DIVIDER_NONE}, {@link #SHOW_DIVIDER_BEGINNING},
	 * {@link #SHOW_DIVIDER_MIDDLE} and {@link #SHOW_DIVIDER_END}
	 */
	private int mShowDividerHorizontal;

	/**
	 * Indicates the divider mode for the {@link #mDividerDrawableVertical}. The
	 * value needs to be the combination of the value of
	 * {@link #SHOW_DIVIDER_NONE}, {@link #SHOW_DIVIDER_BEGINNING},
	 * {@link #SHOW_DIVIDER_MIDDLE} and {@link #SHOW_DIVIDER_END}
	 */
	...
	/**
	 * Holds reordered indices, which {@link LayoutParams#order} parameters are
	 * taken into account
	 */
	private int[] mReorderedIndices;

	/**
	 * Caches the {@link LayoutParams#order} attributes for children views. Key:
	 * the index of the view ({@link #mReorderedIndices} isn't taken into
	 * account) Value: the value for the order attribute
	 */
	...
	/**
	 * Holds the 'frozen' state of children during measure. If a view is frozen
	 * it will no longer expand or shrink regardless of flexGrow/flexShrink.
	...
	public FlexboxLayout(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		int dividerModeVertical = a.getInt(R.styleable.FlexboxLayout_showDividerVertical, SHOW_DIVIDER_NONE);
		if (dividerModeVertical != SHOW_DIVIDER_NONE) {
			mShowDividerVertical = dividerModeVertical;
		}
		int dividerModeHorizontal = a.getInt(R.styleable.FlexboxLayout_showDividerHorizontal, SHOW_DIVIDER_NONE);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// TODO: Only calculate the children views which are affected from the
		// last measure.
		...
		case FLEX_DIRECTION_COLUMN_REVERSE:
			...
			break;
		default:
			throw new IllegalStateException("Invalid value for the flex direction is set: " + mFlexDirection);
		}
	...
	/**
	 * Returns a View, which is reordered by taking {@link LayoutParams#order}
	 * parameters into account.
	 * 
	 * @param index
	 *            the index of the view
	 * @return the reordered view, which {@link LayoutParams@order} is taken
	 *         into account. If the index is negative or out of bounds of the
	 *         number of contained views, returns {@code null}.
	 */
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		// Create an array for the reordered indices before the View is added in
		// the parent
		// ViewGroup since otherwise reordered indices won't be in effect before
		// the
	 ...
	 * {@link LayoutParams#order} attributes are taken into account. This method
	 * takes a View before that is added as the parent ViewGroup's children.
	 * 
	 * @param viewBeforeAdded
	 *            the View instance before added to the array of children Views
	 *            of the parent ViewGroup
	 * @param indexForViewBeforeAdded
	 *            the index for the View before added to the array of the parent
	 *            ViewGroup
	 * @param paramsForViewBeforeAdded
	 *            the layout parameters for the View before added to the array
	 *            of the parent ViewGroup
	...
	private int[] createReorderedIndices(View viewBeforeAdded, int indexForViewBeforeAdded, ViewGroup.LayoutParams paramsForViewBeforeAdded) {
		...
		if (indexForViewBeforeAdded == -1 || indexForViewBeforeAdded == childCount) {
			...
			// This path is not expected since OutOfBoundException will be
			// thrown in the ViewGroup
	...
	/**
	 * Create an array, which indicates the reordered indices that
	 * {@link LayoutParams#order} attributes are taken into account.
	 * 
	...
	/**
	 * Returns if any of the children's {@link LayoutParams#order} attributes
	 * are changed from the last measurement.
	 * 
	 * @return {@code true} if changed from the last measurement, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * Sub method for {@link #onMeasure(int, int)}, when the main axis direction
	 * is horizontal (either left to right or right to left).
	 * 
	...
	private void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// Determine how many flex lines are needed in this layout by measuring
		// each child.
		// (Expand or shrink the view depending on the flexGrow and flexShrink
		// attributes in a later
		// loop)
		{
			int childCount = getChildCount();
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (lp.flexBasisPercent != LayoutParams.FLEX_BASIS_PERCENT_DEFAULT && widthMode == MeasureSpec.EXACTLY) {
					childWidth = Math.round(widthSize * lp.flexBasisPercent);
					// Use the dimension from the layout_width attribute if the
					// widthMode is not
					// MeasureSpec.EXACTLY even if any fraction value is set to
					// layout_flexBasisPercent.
					// There are likely quite few use cases where assigning any
					// fraction values
					// with widthMode is not MeasureSpec.EXACTLY (e.g.
					// FlexboxLayout's layout_width
					// is set to wrap_content)
				}
				int childWidthMeasureSpec = getChildMeasureSpec(widthMeasureSpec, getPaddingLeft() + getPaddingRight() + lp.leftMargin + lp.rightMargin, childWidth);
				...
				// Check the size constraint after the first measurement for the
				// child
				// To prevent the child's width/height violate the size
				// constraints imposed by the
				// {@link LayoutParams#minWidth}, {@link
				// LayoutParams#minHeight},
				// {@link LayoutParams#maxWidth} and {@link
				// LayoutParams#maxHeight} attributes.
				// E.g. When the child's layout_width is wrap_content the
				// measured width may be
				...
				if (isWrapRequired(widthMode, widthSize, flexLine.mMainSize, child.getMeasuredWidth() + lp.leftMargin + lp.rightMargin, lp, i, indexInFlexLine)) {
				...
				// Temporarily set the cross axis length as the largest child in
				// the row
				// Expand along the cross axis depending on the mAlignContent
				// property if needed
				// later
				flexLine.mCrossSize = Math.max(flexLine.mCrossSize, largestHeightInRow);

				// Check if the beginning or middle divider is required for the
				// flex item
				...
				if (mFlexWrap != FLEX_WRAP_WRAP_REVERSE) {
					...
					// if the flex wrap property is FLEX_WRAP_WRAP_REVERSE,
					// calculate the
					// baseline as the distance from the cross end and the
					// baseline
					// since the cross size calculation is based on the distance
					// from the cross end
		...
		// TODO: Consider the case any individual child's alignSelf is set to
		// ALIGN_SELF_BASELINE
		if (mAlignItems == ALIGN_ITEMS_BASELINE) {
			int viewIndex = 0;
			for (FlexLine flexLine : mFlexLines) {
				// The largest height value that also take the baseline shift
				// into account
		...
		determineCrossSize(mFlexDirection, widthMeasureSpec, heightMeasureSpec, getPaddingTop() + getPaddingBottom());
		// Now cross size for each flex line is determined.
		// Expand the views if alignItems (or alignSelf in each child view) is
		// set to stretch
	...
	/**
	 * Sub method for {@link #onMeasure(int, int)} when the main axis direction
	 * is vertical (either from top to bottom or bottom to top).
	 * 
	 * @param widthMeasureSpec
	 *            horizontal space requirements as imposed by the parent
	 * @param heightMeasureSpec
	...
	private void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.flexBasisPercent != LayoutParams.FLEX_BASIS_PERCENT_DEFAULT && heightMode == MeasureSpec.EXACTLY) {
				childHeight = Math.round(heightSize * lp.flexBasisPercent);
				// Use the dimension from the layout_height attribute if the
				// heightMode is not
				// MeasureSpec.EXACTLY even if any fraction value is set to
				// layout_flexBasisPercent.
				// There are likely quite few use cases where assigning any
				// fraction values
				// with heightMode is not MeasureSpec.EXACTLY (e.g.
				// FlexboxLayout's layout_height
				// is set to wrap_content)
			}
			...
			int childHeightMeasureSpec = getChildMeasureSpec(heightMeasureSpec, getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin, childHeight);
			child.measure(childWidthMeasureSpec, childHeightMeasureSpec);

			// Check the size constraint after the first measurement for the
			// child
			// To prevent the child's width/height violate the size constraints
			// imposed by the
			// {@link LayoutParams#minWidth}, {@link LayoutParams#minHeight},
			// {@link LayoutParams#maxWidth} and {@link LayoutParams#maxHeight}
			// attributes.
			// E.g. When the child's layout_height is wrap_content the measured
			// height may be
			...
			if (isWrapRequired(heightMode, heightSize, flexLine.mMainSize, child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin, lp, i, indexInFlexLine)) {
				if (flexLine.mItemCount > 0) {
					addFlexLine(flexLine);
				}
			...
			flexLine.mTotalFlexGrow += lp.flexGrow;
			flexLine.mTotalFlexShrink += lp.flexShrink;
			// Temporarily set the cross axis length as the largest child width
			// in the column
			// Expand along the cross axis depending on the mAlignContent
			// property if needed
			// later
		...
		determineMainSize(mFlexDirection, widthMeasureSpec, heightMeasureSpec);
		determineCrossSize(mFlexDirection, widthMeasureSpec, heightMeasureSpec, getPaddingLeft() + getPaddingRight());
		// Now cross size for each flex line is determined.
		// Expand the views if alignItems (or alignSelf in each child view) is
		// set to stretch
		stretchViews(mFlexDirection, mAlignItems);
	...
	/**
	 * Checks if the view's width/height don't violate the minimum/maximum size
	 * constraints imposed by the {@link LayoutParams#minWidth},
	 * {@link LayoutParams#minHeight}, {@link LayoutParams#maxWidth} and
	 * {@link LayoutParams#maxHeight} attributes.
	 * 
	 * @param view
	...
	private void addFlexLine(FlexLine flexLine) {
		// The size of the end divider isn't added until the flexLine is added
		// to the flex container
		// take the divider width (or height) into account when adding the flex
		// line.
	...
	/**
	 * Determine the main size by expanding (shrinking if negative remaining
	 * free space is given) an individual child in each flex line if any
	 * children's flexGrow (or flexShrink if remaining space is negative)
	 * properties are set to non-zero.
	 * 
	 * @param flexDirection
	 *            the value of the flex direction
	 * @param widthMeasureSpec
	 *            horizontal space requirements as imposed by the parent
	 * @param heightMeasureSpec
	 *            vertical space requirements as imposed by the parent
	 ...
	 */
	private void determineMainSize(@FlexDirection int flexDirection, int widthMeasureSpec, int heightMeasureSpec) {
		int mainSize;
		...
		for (FlexLine flexLine : mFlexLines) {
			if (flexLine.mMainSize < mainSize) {
				childIndex = expandFlexItems(widthMeasureSpec, heightMeasureSpec, flexLine, flexDirection, mainSize, paddingAlongMainAxis, childIndex, false);
			} else {
				childIndex = shrinkFlexItems(widthMeasureSpec, heightMeasureSpec, flexLine, flexDirection, mainSize, paddingAlongMainAxis, childIndex, false);
			}
		}
	}

	/**
	 * Expand the flex items along the main axis based on the individual
	 * flexGrow attribute.
	 * 
	 * @param widthMeasureSpec
	 ...
	 * @param maxMainSize
	 *            the maximum main size. Expanded main size will be this size
	 * @param paddingAlongMainAxis
	 *            the padding value along the main axis
	 * @param startIndex
	 *            the start index of the children views to be expanded. This
	 *            index needs to be an absolute index in the flex container
	 *            (FlexboxLayout), not the relative index in the flex line.
	 * @param calledRecursively
	 *            true if this method is called recursively, false otherwise
	 * @return the next index, the next flex line's first flex item starts from
	 *         the returned index
	 * @see #getFlexDirection()
	 * @see #setFlexDirection(int)
	 * @see LayoutParams#flexGrow
	 */
	private int expandFlexItems(int widthMeasureSpec, int heightMeasureSpec, FlexLine flexLine, @FlexDirection int flexDirection, int maxMainSize, int paddingAlongMainAxis, int startIndex,
		...
		// Setting the cross size of the flex line as the temporal value since
		// the cross size of
		// each flex item may be changed from the initial calculation
		// (in the measureHorizontal/measureVertical method) even this method is
		// part of the main
		// size determination.
		// E.g. If a TextView's layout_width is set to 0dp, layout_height is set
		// to wrap_content,
		// and layout_flexGrow is set to 1, the TextView is trying to expand to
		// the vertical
		// direction to enclose its content (in the measureHorizontal method),
		// but
		// the width will be expanded in this method. In that case, the height
			...
			if (isMainAxisDirectionHorizontal(flexDirection)) {
				...
				if (!mChildrenFrozen[childIndex]) {
					...
					if (newWidth > lp.maxWidth) {
						// This means the child can't expand beyond the value of
						// the maxWidth attribute.
						// To adjust the flex line length to the size of
						// maxMainSize, remaining
						// positive free space needs to be re-distributed to
						// other flex items
						// (children views). In that case, invoke this method
						// again with the same
						// startIndex.
						needsReexpand = true;
					...
					child.measure(MeasureSpec.makeMeasureSpec(newWidth, MeasureSpec.EXACTLY), childHeightMeasureSpec);
				...
				if (!mChildrenFrozen[childIndex]) {
					...
					child.measure(childWidthMeasureSpec, MeasureSpec.makeMeasureSpec(newHeight, MeasureSpec.EXACTLY));
		...
		if (needsReexpand && sizeBeforeExpand != flexLine.mMainSize) {
			...
			// positive free space
			// that wasn't fully distributed (because of maximum length
			// constraint)
	...
	/**
	 * Shrink the flex items along the main axis based on the individual
	 ...
	 *            the flexDirection value for this FlexboxLayout
	 * @param maxMainSize
	 *            the maximum main size. Shrank main size will be this size
	 * @param paddingAlongMainAxis
	 *            the padding value along the main axis
	 * @param startIndex
	 ...
	 */
	private int shrinkFlexItems(int widthMeasureSpec, int heightMeasureSpec, FlexLine flexLine, @FlexDirection int flexDirection, int maxMainSize, int paddingAlongMainAxis, int startIndex,
			boolean calledRecursively) {
		int childIndex = startIndex;
			...
			if (isMainAxisDirectionHorizontal(flexDirection)) {
				...
				if (!mChildrenFrozen[childIndex]) {
					...
					if (newWidth < lp.minWidth) {
						// This means the child doesn't have enough space to
						// distribute the negative
						// free space. To adjust the flex line length down to
						// the maxMainSize, remaining
						// negative free space needs to be re-distributed to
						// other flex items
						// (children views). In that case, invoke this method
						// again with the same
						// startIndex.
				...
				if (!mChildrenFrozen[childIndex]) {
					...
					if (newHeight < lp.minHeight) {
						// Need to invoke this method again like the case flex
		...
		if (needsReshrink && sizeBeforeShrink != flexLine.mMainSize) {
			// Re-invoke the method with the same startIndex to distribute the
			// negative free space
			// that wasn't fully distributed (because some views length were not
			// enough)
	...
	private int getChildWidthMeasureSpec(int widthMeasureSpec, LayoutParams lp) {
		...
		if (childWidth > lp.maxWidth) {
			childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(lp.maxWidth, MeasureSpec.getMode(childWidthMeasureSpec));
		} else if (childWidth < lp.minWidth) {
			childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(lp.minWidth, MeasureSpec.getMode(childWidthMeasureSpec));
		}
	...
	private int getChildHeightMeasureSpec(int heightMeasureSpec, LayoutParams lp) {
		...
		if (childHeight > lp.maxHeight) {
			childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(lp.maxHeight, MeasureSpec.getMode(childHeightMeasureSpec));
		} else if (childHeight < lp.minHeight) {
			childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(lp.minHeight, MeasureSpec.getMode(childHeightMeasureSpec));
		}
	 ...
	 * Expand the cross size only if the height mode is MeasureSpec.EXACTLY,
	 * otherwise use the sum of cross sizes of all flex lines.
	 * 
	 * @param flexDirection
	 ...
	 * @param heightMeasureSpec
	 *            vertical space requirements as imposed by the parent
	 * @param paddingAlongCrossAxis
	 *            the padding value for the FlexboxLayout along the cross axis
	 * @see #getFlexDirection()
	 * @see #setFlexDirection(int)
	 * @see #getAlignContent()
	 * @see #setAlignContent(int)
	...
	private void determineCrossSize(int flexDirection, int widthMeasureSpec, int heightMeasureSpec, int paddingAlongCrossAxis) {
		...
		if (mode == MeasureSpec.EXACTLY) {
			...
			if (mFlexLines.size() == 1) {
				...
				// alignContent property is valid only if the Flexbox has at
				// least two lines
				...
				case ALIGN_CONTENT_SPACE_AROUND: {
					// The value of free space along the cross axis which needs
					// to be put on top
				...
				case ALIGN_CONTENT_SPACE_BETWEEN: {
					// The value of free space along the cross axis between each
					// flex line.
					...
					for (int i = 0, flexLineSize = mFlexLines.size(); i < flexLineSize; i++) {
						...
						if (i != mFlexLines.size() - 1) {
							...
							if (i == mFlexLines.size() - 2) {
								// The last dummy space block in the flex
								// container.
								// Adjust the cross size by the accumulated
								// error.
	...
	/**
	 * Expand the view if the {@link #mAlignItems} attribute is set to
	 * {@link #ALIGN_ITEMS_STRETCH} or {@link LayoutParams#ALIGN_SELF_STRETCH}
	 * is set to an individual child view.
	 * 
	 * @param flexDirection
	 *            the flex direction attribute
	 * @param alignItems
	 *            the align items attribute
	 * @see #getFlexDirection()
	 * @see #setFlexDirection(int)
	 * @see #getAlignItems()
	 * @see #setAlignItems(int)
		...
		if (alignItems == ALIGN_ITEMS_STRETCH) {
			...
			for (FlexLine flexLine : mFlexLines) {
				for (int i = 0; i < flexLine.mItemCount; i++, viewIndex++) {
					...
					LayoutParams lp = (LayoutParams) view.getLayoutParams();
					if (lp.alignSelf != LayoutParams.ALIGN_SELF_AUTO && lp.alignSelf != LayoutParams.ALIGN_SELF_STRETCH) {
						continue;
					}
					switch (flexDirection) {
					case FLEX_DIRECTION_ROW: // Intentional fall through
	...
	/**
	 * Expand the view vertically to the size of the crossSize (considering the
	 * view margins)
	 * 
	 * @param view
	 *            the View to be stretched
	 * @param crossSize
	 *            the cross size
	 */
	...
	/**
	 * Expand the view horizontally to the size of the crossSize (considering
	 * the view margins)
	 * 
	 * @param view
	 *            the View to be stretched
	...
	private void stretchViewHorizontally(View view, int crossSize) {
		...
		view.measure(MeasureSpec.makeMeasureSpec(newWidth, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(view.getMeasuredHeight(), MeasureSpec.EXACTLY));
	}

	/**
	 * Set this FlexboxLayouts' width and height depending on the calculated
	 * size of main axis and cross axis.
	 * 
	 * @param flexDirection
	 *            the value of the flex direction
	 * @param widthMeasureSpec
	 ...
	 * @see #setFlexDirection(int)
	 */
	private void setMeasuredDimensionForFlex(@FlexDirection int flexDirection, int widthMeasureSpec, int heightMeasureSpec, int childState) {
		int widthMode = MeasureSpec.getMode(widthMeasureSpec);
		int widthSize = MeasureSpec.getSize(widthMeasureSpec);
		int heightMode = MeasureSpec.getMode(heightMeasureSpec);
		int heightSize = MeasureSpec.getSize(heightMeasureSpec);
		...
		case MeasureSpec.EXACTLY:
			if (widthSize < calculatedMaxWidth) {
				childState = ViewCompat.combineMeasuredStates(childState, ViewCompat.MEASURED_STATE_TOO_SMALL);
			}
		...
		case MeasureSpec.EXACTLY:
			if (heightSize < calculatedMaxHeight) {
				childState = ViewCompat.combineMeasuredStates(childState, ViewCompat.MEASURED_STATE_TOO_SMALL >> ViewCompat.MEASURED_HEIGHT_STATE_SHIFT);
			}
	 ...
	 * 
	 * @param mode
	 *            the width or height mode along the main axis direction
	 * @param maxSize
	 *            the max size along the main axis direction
	 * @param currentLength
	 *            the accumulated current length
	 * @param childLength
	 *            the length of a child view which is to be collected to the
	 *            flex line
	 * @param lp
	 *            the LayoutParams for the view being determined whether a new
	 *            flex line is needed
	...
	/**
	 * Retrieve the sum of the cross sizes of all flex lines including divider
	 * lengths.
	 * 
	 ...
	 * 
	 * @param isRtl
	 *            {@code true} if the horizontal layout direction is right to
	 *            left, {@code false} otherwise.
	 * @param left
	 *            the left position of this View
	 * @param top
	 *            the top position of this View
	 * @param right
	 *            the right position of this View
	 * @param bottom
	 *            the bottom position of this View
	 * @see #getFlexWrap()
	 * @see #setFlexWrap(int)
	...
	private void layoutHorizontal(boolean isRtl, int left, int top, int right, int bottom) {
		...
		// childBottom is used if the mFlexWrap is FLEX_WRAP_WRAP_REVERSE
		// otherwise
		// childTop is used to align the vertical position of the children
		// views.
		...
		for (int i = 0, size = mFlexLines.size(); i < size; i++) {
			...
			case JUSTIFY_CONTENT_SPACE_AROUND:
				if (flexLine.mItemCount != 0) {
					spaceBetweenItem = (width - flexLine.mMainSize) / (float) flexLine.mItemCount;
				}
			...
			for (int j = 0; j < flexLine.mItemCount; j++) {
				...
				if (mFlexWrap == FLEX_WRAP_WRAP_REVERSE) {
					if (isRtl) {
						layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems, Math.round(childRight) - child.getMeasuredWidth(), childBottom - child.getMeasuredHeight(),
								Math.round(childRight), childBottom);
					} else {
						layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems, Math.round(childLeft), childBottom - child.getMeasuredHeight(),
								Math.round(childLeft) + child.getMeasuredWidth(), childBottom);
					}
				} else {
					if (isRtl) {
						layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems, Math.round(childRight) - child.getMeasuredWidth(), childTop, Math.round(childRight),
								childTop + child.getMeasuredHeight());
					} else {
						layoutSingleChildHorizontal(child, flexLine, mFlexWrap, mAlignItems, Math.round(childLeft), childTop, Math.round(childLeft) + child.getMeasuredWidth(),
								childTop + child.getMeasuredHeight());
					}
				}
	 ...
	 * 
	 * @param view
	 *            the View to be placed
	 * @param flexLine
	 *            the {@link FlexLine} where the View belongs to
	 * @param flexWrap
	 *            the flex wrap attribute of this FlexboxLayout
	 * @param alignItems
	 *            the align items attribute of this FlexboxLayout
	 * @param left
	 *            the left position of the View, which the View's margin is
	 *            already taken into account
	 * @param top
	 *            the top position of the flex line where the View belongs to.
	 *            The actual View's top position is shifted depending on the
	 *            flexWrap and alignItems attributes
	 * @param right
	 *            the right position of the View, which the View's margin is
	 *            already taken into account
	 * @param bottom
	 *            the bottom position of the flex line where the View belongs
	 *            to. The actual View's bottom position is shifted depending on
	 *            the flexWrap and alignItems attributes
	 * @see #getAlignItems()
	 * @see #setAlignItems(int)
	 * @see LayoutParams#alignSelf
	 */
	private void layoutSingleChildHorizontal(View view, FlexLine flexLine, @FlexWrap int flexWrap, int alignItems, int left, int top, int right, int bottom) {
		LayoutParams lp = (LayoutParams) view.getLayoutParams();
		if (lp.alignSelf != LayoutParams.ALIGN_SELF_AUTO) {
			// Expecting the values for alignItems and alignSelf match except
			// for ALIGN_SELF_AUTO.
		...
		case ALIGN_ITEMS_FLEX_END:
			if (flexWrap != FLEX_WRAP_WRAP_REVERSE) {
				...
				// If the flexWrap == FLEX_WRAP_WRAP_REVERSE, the direction of
				// the
	 ...
	 *            {@code true} if the layout direction is bottom to top,
	...
	private void layoutVertical(boolean isRtl, boolean fromBottomToTop, int left, int top, int right, int bottom) {
		...
		int width = right - left;
		int height = bottom - top;
		// childRight is used if the mFlexWrap is FLEX_WRAP_WRAP_REVERSE
		// otherwise
		...
		for (int i = 0, size = mFlexLines.size(); i < size; i++) {
			...
			case JUSTIFY_CONTENT_SPACE_AROUND:
				if (flexLine.mItemCount != 0) {
					spaceBetweenItem = (height - flexLine.mMainSize) / (float) flexLine.mItemCount;
				}
			...
			for (int j = 0; j < flexLine.mItemCount; j++) {
				...
				if (isRtl) {
					if (fromBottomToTop) {
						layoutSingleChildVertical(child, flexLine, true, mAlignItems, childRight - child.getMeasuredWidth(), Math.round(childBottom) - child.getMeasuredHeight(), childRight,
								Math.round(childBottom));
					} else {
						layoutSingleChildVertical(child, flexLine, true, mAlignItems, childRight - child.getMeasuredWidth(), Math.round(childTop), childRight,
								Math.round(childTop) + child.getMeasuredHeight());
					}
				} else {
					if (fromBottomToTop) {
						layoutSingleChildVertical(child, flexLine, false, mAlignItems, childLeft, Math.round(childBottom) - child.getMeasuredHeight(), childLeft + child.getMeasuredWidth(),
								Math.round(childBottom));
					} else {
						layoutSingleChildVertical(child, flexLine, false, mAlignItems, childLeft, Math.round(childTop), childLeft + child.getMeasuredWidth(),
								Math.round(childTop) + child.getMeasuredHeight());
					}
				}
	 ...
	 *            the {@link FlexLine} where the View belongs to
	 * @param isRtl
	 *            {@code true} if the layout direction is right to left,
	 *            {@code false} otherwise
	 * @param alignItems
	 *            the align items attribute of this FlexboxLayout
	 * @param left
	 *            the left position of the flex line where the View belongs to.
	 *            The actual View's left position is shifted depending on the
	 *            isRtl and alignItems attributes
	 * @param top
	 *            the top position of the View, which the View's margin is
	 *            already taken into account
	 * @param right
	 *            the right position of the flex line where the View belongs to.
	 *            The actual View's right position is shifted depending on the
	 *            isRtl and alignItems attributes
	 * @param bottom
	 *            the bottom position of the View, which the View's margin is
	 *            already taken into account
	 * @see #getAlignItems()
	 * @see #setAlignItems(int)
	 * @see LayoutParams#alignSelf
	...
	/**
	 * Sub method for {@link #onDraw(Canvas)} when the main axis direction is
	 * horizontal ({@link #mFlexDirection} is either of
	 * {@link #FLEX_DIRECTION_ROW} or {@link #FLEX_DIRECTION_ROW_REVERSE}.
	 * 
	 * @param canvas
	 *            the canvas on which the background will be drawn
	 * @param isRtl
	 *            {@code true} when the horizontal layout direction is right to
	 *            left, {@code false} otherwise
	 * @param fromBottomToTop
	 *            {@code true} when the vertical layout direction is bottom to
	 *            top, {@code false} otherwise
	 */
	private void drawDividersHorizontal(Canvas canvas, boolean isRtl, boolean fromBottomToTop) {
		...
		for (int i = 0, size = mFlexLines.size(); i < size; i++) {
			...
			// Judge if the beginning or middle dividers are needed before the
			// flex line
			if (hasDividerBeforeFlexLine(i)) {
	...
	/**
	 * Sub method for {@link #onDraw(Canvas)} when the main axis direction is
	 * vertical ({@link #mFlexDirection} is either of
	 * {@link #FLEX_DIRECTION_COLUMN} or {@link #FLEX_DIRECTION_COLUMN_REVERSE}.
	 * 
	 * @param canvas
	 *            the canvas on which the background will be drawn
	 * @param isRtl
	...
	/**
	 * @return the flex lines composing this flex container. This method returns
	 *         a copy of the original list excluding a dummy flex line (flex
	 *         line that doesn't have any flex items in it but used for the
	 *         alignment along the cross axis). Thus any changes of the returned
	 *         list are not reflected to the original list.
	 */
	...
	/**
	 * Set a drawable to be used as a divider between items. The drawable is
	 * used for both horizontal and vertical dividers.
	 * 
	 * @param divider
	 *            Drawable that will divide each item for both horizontally and
	 *            vertically.
	...
	/**
	 * Set how dividers should be shown between items in this layout. This
	 * method sets the divider mode for both horizontally and vertically.
	 * 
	 * @param dividerMode
	...
	/**
	 * Check if a divider is needed before the view whose indices are passed as
	 * arguments.
	 * 
	 * @param childAbsoluteIndex
	 *            the absolute index of the view to be judged
	 * @param childRelativeIndexInFlexLine
	 *            the relative index in the flex line where the view belongs
	...
	/**
	 * Check if a divider is needed before the flex line whose index is passed
	 * as an argument.
	 * 
	 * @param flexLineIndex
	 *            the index of the flex line to be checked
	 * @return {@code true} if a divider is needed, {@code false} otherwise
	 */
	...
	/**
	 * Check if a end divider is needed after the flex line whose index is
	...
	public static class LayoutParams extends ViewGroup.MarginLayoutParams {
		...
		/**
		 * This attribute can change the ordering of the children views are laid
		 * out. By default, children are displayed and laid out in the same
		 * order as they appear in the layout XML. If not specified,
		 * {@link #ORDER_DEFAULT} is set as a default value.
		 */
		public int order = ORDER_DEFAULT;

		/**
		 * This attribute determines how much this child will grow if positive
		 * free space is distributed relative to the rest of other flex items
		 * included in the same flex line. If not specified,
		 * {@link #FLEX_GROW_DEFAULT} is set as a default value.
		 */
		public float flexGrow = FLEX_GROW_DEFAULT;

		/**
		 * This attributes determines how much this child will shrink is
		 * negative free space is distributed relative to the rest of other flex
		 * items included in the same flex line. If not specified,
		 * {@link #FLEX_SHRINK_DEFAULT} is set as a default value.
		 */
		public float flexShrink = FLEX_SHRINK_DEFAULT;

		/**
		 * This attributes determines the alignment along the cross axis
		 * (perpendicular to the main axis). The alignment in the same direction
		 * can be determined by the {@link #mAlignItems} in the parent, but if
		 * this is set to other than {@link #ALIGN_SELF_AUTO}, the cross axis
		 * alignment is overridden for this child. The value needs to be one of
		 * the values in ({@link #ALIGN_SELF_AUTO}, {@link #ALIGN_SELF_STRETCH},
		 * {@link #ALIGN_SELF_FLEX_START}, {@link #ALIGN_SELF_FLEX_END},
		 * {@link #ALIGN_SELF_CENTER}, or {@link #ALIGN_SELF_BASELINE}). If not
		 * specified, {@link #ALIGN_SELF_AUTO} is set as a default value.
		 */
		public int alignSelf = ALIGN_SELF_AUTO;

		/**
		 * The initial flex item length in a fraction format relative to its
		 * parent. The initial main size of this child View is trying to be
		 * expanded as the specified fraction against the parent main size. If
		 * this value is set, the length specified from layout_width (or
		 * layout_height) is overridden by the calculated value from this
		 * attribute. This attribute is only effective when the parent's
		 * MeasureSpec mode is MeasureSpec.EXACTLY. The default value is
		 * {@link #FLEX_BASIS_PERCENT_DEFAULT}, which means not set.
		 */
		...
		/**
		 * This attribute forces a flex line wrapping. i.e. if this is set to
		 * {@code true} for a flex item, the item will become the first item of
		 * the new flex line. (A wrapping happens regardless of the flex items
		 * being processed in the the previous flex line) This attribute is
		 * ignored if the flex_wrap attribute is set as nowrap. The equivalent
		 * attribute isn't defined in the original CSS Flexible Box Module
		 * specification, but having this attribute is useful for Android
		 * developers to flatten the layouts when building a grid like layout or
		 * for a situation where developers want to put a new flex line to make
		 * a semantic difference from the previous one, etc.
		 */
		...
		public LayoutParams(Context context, AttributeSet attrs) {
			...
			flexShrink = a.getFloat(R.styleable.FlexboxLayout_Layout_layout_flexShrink, FLEX_SHRINK_DEFAULT);
			alignSelf = a.getInt(R.styleable.FlexboxLayout_Layout_layout_alignSelf, ALIGN_SELF_AUTO);
			flexBasisPercent = a.getFraction(R.styleable.FlexboxLayout_Layout_layout_flexBasisPercent, 1, 1, FLEX_BASIS_PERCENT_DEFAULT);
			...
			maxWidth = a.getDimensionPixelSize(R.styleable.FlexboxLayout_Layout_layout_maxWidth, MAX_SIZE);
			maxHeight = a.getDimensionPixelSize(R.styleable.FlexboxLayout_Layout_layout_maxHeight, MAX_SIZE);
	...
	/**
	 * A class that is used for calculating the view order which view's indices
	 * and order properties from Flexbox are taken into account.
	 */
