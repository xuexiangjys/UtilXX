...
public class BottomDialog {
    ...
    public BottomDialog addItems(List<ActionItem> items, OnItemClickListener onItemClickListener) {
    ...
    private final class CustomDialog extends Dialog {
        ...
        void addItems(List<ActionItem> items, OnItemClickListener onItemClickListener) {
        ...
        void inflateMenu(int menu, OnItemClickListener onItemClickListener) {
            ...
            for (int i = 0; i < menuBuilder.size(); i++) {
                ...
                items.add(new ActionItem(menuItem.getItemId(), menuItem.getTitle().toString(), menuItem.getIcon()));
            }
        ...
        private class DialogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
            ...
            private List<ActionItem> mItems = Collections.emptyList();
            private OnItemClickListener itemClickListener;

            private int orientation;
            private int layout;

            DialogAdapter(List<ActionItem> mItems, int layout, int orientation) {
                setList(mItems);
                this.layout = layout;
                this.orientation = orientation;
            }

            private void setList(List<ActionItem> items) {
                mItems = items == null ? new ArrayList<ActionItem>() : items;
            }
            ...
            class LeftHolder extends RecyclerView.ViewHolder {
                ...
                private Drawable icon(Drawable drawable) {
                    if (drawable != null) {
                        Bitmap bitmap = ((BitmapDrawable) drawable).getBitmap();
                        Drawable resizeIcon = new BitmapDrawable(getContext().getResources(), Bitmap.createScaledBitmap(bitmap, leftIcon, leftIcon, true));
                        Drawable.ConstantState state = resizeIcon.getConstantState();
                        resizeIcon = DrawableCompat.wrap(state == null ? resizeIcon : state.newDrawable().mutate());
                        return resizeIcon;
                    }
    ...
    public class ActionItem {
        ...
        public ActionItem(int id, String title, Drawable icon) {
            this.id = id;
            this.title = title;
            this.icon = icon;
        }
