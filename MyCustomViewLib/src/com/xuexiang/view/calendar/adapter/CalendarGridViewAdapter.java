package com.xuexiang.view.calendar.adapter;
...
public class CalendarGridViewAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView,
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.adapter_calendar_item,
					parent, false);
		...
		if (!TextUtils.isEmpty(calendarDate.getSolar().solar24Term)) {
			str = calendarDate.getSolar().solar24Term;
				...
				.isEmpty(calendarDate.getSolar().solarFestivalName)) {
			str = calendarDate.getSolar().solarFestivalName;
		} else {
			str = calendarDate.getLunar().getChinaDayString(
					mListData.get(position).getLunar().lunarDay);
		}
