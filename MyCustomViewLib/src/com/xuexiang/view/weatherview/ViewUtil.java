package com.xuexiang.view.weatherview;
...
public class ViewUtil {
    ...
    public static int scaleValue(Context context, float value) {
        ...
        if (mDisplayMetrics.scaledDensity >= AppConsts.UI_DENSITY) {
            if (width > AppConsts.UI_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (width < AppConsts.UI_WIDTH) {
                value = value * (1.0f - 1.0f / mDisplayMetrics.scaledDensity);
            }
        } else {
            float offset = AppConsts.UI_DENSITY - mDisplayMetrics.scaledDensity;
            if (offset > 0.5f) {
                value = value * 0.9f;
            } else {
                value = value * 0.95f;
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        try {
            ...
            float scaleWidth = (float) width / AppConsts.UI_WIDTH;
            float scaleHeight = (float) height / AppConsts.UI_HEIGHT;
