...
/**
 * You can use a vector drawable in TextView instead of drawableLeft,
 * drawableTop, drawableRight and drawableBottom.
 * <p>
...
public class CompoundIconTextView extends AppCompatTextView {
	...
	public CompoundIconTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
		...
		if (drawableResIds[INDEX_LEFT] == UNDEFINED_RESOURCE && drawableResIds[INDEX_TOP] == UNDEFINED_RESOURCE && drawableResIds[INDEX_RIGHT] == UNDEFINED_RESOURCE
	...
	/**
	 * @param resourceId
	 *            Set the {@link CompoundIconTextView#UNDEFINED_RESOURCE} if you
	 *            want clear icon.
	 */
	 ...
	 * 
	 * @param widthRes
	 *            Set width resource id
	 * @param heightRes
	 *            Set height resource id
	 */
	public void setIconSizeResource(@DimenRes final int widthRes, @DimenRes final int heightRes) {
		setIconSize(getContext().getResources().getDimensionPixelSize(widthRes), getContext().getResources().getDimensionPixelSize(heightRes));
	}
	 ...
	 *            Set height size
	 */
	public void setIconSize(@DimenRes final int width, @DimenRes final int height) {
		iconWidth = width;
		iconHeight = height;
		makeDrawableIcons();
		updateIcons();
	...
	private void updateIcons() {
		setCompoundDrawablesWithIntrinsicBounds(drawables[INDEX_LEFT], drawables[INDEX_TOP], drawables[INDEX_RIGHT], drawables[INDEX_BOTTOM]);
	}
	 ...
	 */
	private Drawable resource2VectorDrawable(@DrawableRes final int resourceId, @ColorInt final int iconColor, final int iconWidth, final int iconHeight) {
		final Context context = getContext();
		final Drawable drawable = context.getResources().getDrawable(resourceId);
		...
		// Resize Bitmap
		return new BitmapDrawable(context.getResources(), Bitmap.createScaledBitmap(drawable2Bitmap(drawable, iconWidth, iconHeight), iconWidth, iconHeight, true));
	}
	...
	/**
	 * @see android.support.v7.widget.DrawableUtils#fixDrawable(Drawable) <p>
	 *      Attempt the fix any issues in the given drawable, usually caused by
	 *      platform bugs in the implementation. This method should be call
	 *      after retrieval from {@link android.content.res.Resources} or a
	 *      {@link android.content.res.TypedArray}.
	 */
	private static void fixDrawable(@NonNull final Drawable drawable) {
		if (Build.VERSION.SDK_INT == Build.VERSION_CODES.LOLLIPOP && VECTOR_DRAWABLE_CLAZZ_NAME.equals(drawable.getClass().getName())) {
	 ...
	 *      <p>
	 *      VectorDrawable has an issue on API 21 where it sometimes doesn't
	 *      create its tint filter. Fixed by toggling it's state to force a
	 *      filter creation.
	 */
	...
	private boolean isRtl() {
		...
		return TextUtilsCompat.getLayoutDirectionFromLocale(resources.getConfiguration().locale) == ViewCompat.LAYOUT_DIRECTION_RTL;
	}
}