...
public class TabView extends RelativeLayout {
	...
	/**
	 * if need splitLine,default height is false
	 */
	private boolean mIsNeedSplitLine;
	/**
	 * the splitLine`s color, default color is LightGray
	 */
	...
	private void initDefaultAttrs(Context context) {
		...
		mTabViewHeight = DisplayUtils.dip2px(context, 52);
		mImageViewTextViewMargin = DisplayUtils.dip2px(context, 2);
		mTextViewSize = DisplayUtils.sp2px(context, 14);
		mImageViewWidth = DisplayUtils.dip2px(context, 30);
		mImageViewHeight = DisplayUtils.dip2px(context, 30);
		mIsNeedSplitLine = false;
		mSplitLineColor = Color.rgb(211, 211, 211);
	}
	...
	private void initCustomAttr(int attr, TypedArray typedArray) {
		...
		} else if (attr == R.styleable.TabView_tab_isNeedSplitLine) {
			mIsNeedSplitLine = typedArray.getBoolean(attr, mIsNeedSplitLine);
		} else if (attr == R.styleable.TabView_tab_splitLineColor) {
			mSplitLineColor = typedArray.getColor(attr, mSplitLineColor);
		}
	}
	...
	private void initView(Context context) {
		...
		RelativeLayout.LayoutParams splitLineParams = null;
		if (mTabViewGravity == Gravity.BOTTOM) {
			tabviewParams = new RelativeLayout.LayoutParams(RMP, mTabViewHeight);
			splitLineParams = new RelativeLayout.LayoutParams(RMP, DisplayUtils.dip2px(context, 1));
			...
			splitLineParams.addRule(RelativeLayout.ABOVE, R.id.tabview_id);
		} else if (mTabViewGravity == Gravity.LEFT) {
			tabviewParams = new RelativeLayout.LayoutParams(mTabViewHeight, RMP);
			splitLineParams = new RelativeLayout.LayoutParams(DisplayUtils.dip2px(context, 1), RMP);
			...
			splitLineParams.addRule(RelativeLayout.RIGHT_OF, R.id.tabview_id);
			...
			splitLineParams.addRule(RelativeLayout.BELOW, R.id.tabview_id);
			...
			splitLineParams.addRule(RelativeLayout.LEFT_OF, R.id.tabview_id);
		}
		...
		if (mIsNeedSplitLine) {
			...
			splitLine.setBackgroundColor(mSplitLineColor);
			splitLine.setLayoutParams(splitLineParams);
			addView(splitLine);
		}
	}
	...
	public void setTabViewChild(List<TabViewChild> tabViewChildList, FragmentManager supportFragmentManager) {
		mTabViewChildList = tabViewChildList;
	...
	public void setImageViewTextViewMargin(int margin) {
		mImageViewTextViewMargin = margin;
	}

	public void setTextViewSize(int size) {
		mTextViewSize = DisplayUtils.sp2px(getContext(), size);
	}
