package com.xuexiang.view.smilerating;
...
public class SmileRating extends BaseRating {
	...
	private void parseAttrs(AttributeSet attrs) {
		if (attrs != null) {
			...
			mAngryColor = a.getColor(R.styleable.SmileRating_sr_angryColor, mAngryColor);
			mNormalColor = a.getColor(R.styleable.SmileRating_sr_normalColor, mNormalColor);
			mDrawingColor = a.getColor(R.styleable.SmileRating_sr_drawingColor, mDrawingColor);
			mPlaceHolderSmileColor = a.getColor(R.styleable.SmileRating_sr_placeHolderSmileColor, mPlaceHolderSmileColor);
			mPlaceholderBackgroundColor = a.getColor(R.styleable.SmileRating_sr_placeHolderBackgroundColor, mPlaceholderBackgroundColor);
			mTextSelectedColor = a.getColor(R.styleable.SmileRating_sr_textSelectionColor, mTextSelectedColor);
			mTextNonSelectedColor = a.getColor(R.styleable.SmileRating_sr_textNonSelectionColor, mTextNonSelectedColor);
			mShowLine = a.getBoolean(R.styleable.SmileRating_sr_showLine, true);
			a.recycle();
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		for (Face face : mFaces) {
			...
			mTextPaint.setColor((int) mColorEvaluator.evaluate(((transY / 0.15f) - 0.2f) / 0.8f, mTextNonSelectedColor, mTextSelectedColor));
			drawTextCentered(getSmileName(face.smileType), face.place.x, face.place.y + (mHeight * (0.70f + transY)), mTextPaint, canvas);
		}
		if (!mSmilePath.isEmpty()) {
			if (mSmileyNotSelectedPreviously) {
				Log.i(TAG, "Non selection");
				/*
				 * mPathPaint.setAlpha(Math.round(255 *
				 * mMainSmileyTransformaFraction));
				 * mBackgroundPaint.setAlpha(Math.round(255 *
				 * mMainSmileyTransformaFraction));
				 */
				mPathPaint.setColor((Integer) mColorEvaluator.evaluate(mMainSmileyTransformaFraction, mPlaceHolderFacePaint.getColor(), mDrawingColor));
				mBackgroundPaint.setColor((Integer) mColorEvaluator.evaluate(mMainSmileyTransformaFraction, mPlaceHolderCirclePaint.getColor(), mNormalColor));
				mScaleMatrix.reset();
				mSmilePath.computeBounds(mScaleRect, true);
				float nonSelectedScale = mFloatEvaluator.evaluate(mInterpolator.getInterpolation(mMainSmileyTransformaFraction), getScale(NONE), 1f);
				mScaleMatrix.setScale(nonSelectedScale, nonSelectedScale, mScaleRect.centerX(), mScaleRect.centerY());
				mDummyDrawPah.reset();
	...
	public void setAngryColor(@ColorInt int color) {
		...
		getSmiley(mSmileys, getFractionBySmiley(mSelectedSmile), divisions, mFromRange, mToRange, mFaceCenter, mSmilePath, mCenterY);
	}
	 ...
	 * 
	 * @param smile
	 *            is the smiley type you want this rating bar to show as
	 *            selected smile
	 */
	 ...
	 *            selected smile
	 * @param animate
	 *            true if you want to set the selected smiley and animate it,
	...
	private void getSmiley(Smileys smileys, float fraction, float divisions, float fromRange, float toRange, Point point, Path smilePath, float centerY) {
		if (smileys == null) {
			return;
		}
		...
		if (fraction > 0.75f) {
			...
			transformSmile(trans, fraction, smilePath, smileys.getSmile(GOOD), smileys.getSmile(GREAT), mFloatEvaluator);
			...
			transformSmile(trans, fraction, smilePath, smileys.getSmile(TERRIBLE), smileys.getSmile(BAD), mFloatEvaluator);
	...
	private static class Face {
		...
		int smileType;
		/*
		 * Path leftEye = new Path(); Path rightEye = new Path();
		 */
	}
	...
	public interface OnSmileySelectionListener {
		 ...
		 * 
		 * @param smiley
		 *            is the type of smiley the user selected ({@link #GREAT},
		 *            {@link #BAD}, {@link #OKAY},{@link #GOOD},{@link #GREAT})
		 * @param reselected
		 *            is false when user selects different smiley that
		 *            previously selected one true when the same smiley is
		 *            selected. Except if it first time, then the value will be
		 *            false.
		 */
	...
	public interface OnRatingSelectedListener {
		 ...
		 * 
		 * @param level
		 *            is the level of the rating (0 to 4)
		 * @param reselected
		 *            is false when user selects different smiley that
		 *            previously selected one true when the same smiley is
		 *            selected. Except if it first time, then the value will be
