...
public class UploadServlet extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		{	
			...
			// 设置文件上传路径
			String uploadPath = this.getServletContext().getRealPath("/uploadPath/");
			...
			for (FileItem item : items)
			{
				...
				if (item.isFormField())					
				{
					if (item.getFieldName().equals("content")){
						System.out.println(inputStream2String(item.getInputStream()));
			...
			if (!filename.equals(""))
			{
				...
				System.out.println("文件保存路径为:" + filename);
