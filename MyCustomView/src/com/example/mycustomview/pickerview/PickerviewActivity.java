...
import com.example.mycustomview.pickerview.bean.CardBean;
import com.example.mycustomview.pickerview.bean.PickerViewData;
import com.example.mycustomview.pickerview.bean.ProvinceBean;
...
import com.xuexiang.view.pickerview.TimePickerView;
import com.xuexiang.view.pickerview.listener.CustomListener;
import com.xuexiang.view.pickerview.weight.WheelView;

public class PickerviewActivity extends BaseHeadActivity implements View.OnClickListener {
	...
	public void onClick(View v) {
		if (v.getId() == R.id.btn_Time && pvTime != null) {
			...
			startActivity(new Intent(PickerviewActivity.this, ProvinceJsonDataActivity.class));
		}
	}
	...
	private void initTimePicker() {
		 ...
		 * .setCancelText("Cancel") .setSubmitText("Sure") .setContentSize(18)
		 * .setTitleSize(20) .setTitleText("Title") .isCyclic(true)// default is
		 * false .setTitleColor(Color.BLACK)
		...
		.setLabel("", "", "", "", "", "") // 设置空字符串以隐藏单位提示 hide label
				.isDialog(false).setOutSideCancelable(true)// default is true
				.setDividerColor(Color.BLACK).setContentSize(20).setDate(selectedDate).setRangDate(startDate, endDate).build();
	}
	...
	private void initCustomTimePicker() {
		...
		}).setType(TimePickerView.Type.YEAR_MONTH_DAY).setDate(selectedDate).setRangDate(startDate, endDate).setLayoutRes(R.layout.layout_pickerview_custom_time, new CustomListener() {
		...
		}).setDividerColor(Color.BLACK).build();
	}
	...
	private void initOptionPicker() {// 条件选择器初始化
		...
		pvOptions = new OptionsPickerView.Builder(this, new OptionsPickerView.OnOptionsSelectListener() {
			...
			public void onOptionsSelect(int options1, int options2, int options3, View v) {
				// 返回的分别是三个级别的选中位置
				String tx = options1Items.get(options1).getPickerViewText() + options2Items.get(options1).get(options2) + options3Items.get(options1).get(options2).get(options3).getPickerViewText();
				btn_Options.setText(tx);
			}
		})
		/*
		 * .setSubmitText("确定") .setCancelText("取消") .setTitleText("城市选择")
		 * .setTitleSize(20) .setSubCalSize(18)//确定取消按钮大小
		 * .setTitleColor(Color.BLACK) .setSubmitColor(Color.BLUE)
		 * .setCancelColor(Color.BLUE) .setBackgroundColor(Color.WHITE)
		 * .setLinkage(false)//default is true .setCyclic(false, false,
		 * false)//循环与否 .setOutSideCancelable(false)//点击外部dismiss, default true
		 * .setTitleBgColor(0xFF333333)//标题背景颜色 Night mode
		 * .setBgColor(0xFF000000)//滚轮背景颜色 Night mode .setLabels("省", "市",
		 * "区")//设置选择的三级单位 .setLineSpacingMultiplier(2.0f) //设置两横线之间的间隔倍数（范围：1.2
		 * - 2.0倍 文字高度） .setDividerColor(Color.RED)//设置分割线的颜色
		...
		.setTypeface(Typeface.SANS_SERIF)// 字体样式
				.setTitleText("城市选择").setDividerType(WheelView.DividerType.WRAP).setContentTextSize(20)// 设置滚轮文字大小
				.isDialog(true).setOutSideCancelable(false)// default is true
				.build();
