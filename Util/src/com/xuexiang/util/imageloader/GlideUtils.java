...
/**
 * Glide 工具类 Google推荐的图片加载库，专注于流畅的滚动
 * <p>
 * Glide 比Picasso 加载快 但需要更大的内存来缓存
 * <p>
 * Glide 不光接受Context，还接受Activity 和 Fragment ,图片加载会和Activity/Fragment的生命周期保持一致
 * 在onPause（）暂停加载，onResume（）恢复加载
 * <p>
 * 支持GIF格式图片加载
 */
public class GlideUtils {
	...
	// load(Uri uri)，load(File file)，load(Integer resourceId)，load(URL
	// url)，load(byte[] model, final String id)，load(byte[] model)，load(T
	// model)。
	// 而且可以使用自己的ModelLoader进行资源加载：
	// using(ModelLoader<A, T> modelLoader, Class<T> dataClass)，using(final
	// StreamModelLoader<T> modelLoader)，using(StreamByteArrayLoader
	// modelLoader)，using(final FileDescriptorModelLoader<T> modelLoader)。
	// 返回RequestBuilder实例
	// --------------------------------------
	...
	// 默认采用DiskCacheStrategy.RESULT策略，对于download
	// only操作要使用DiskCacheStrategy.SOURCE。
	...
	// * listener(RequestListener<? super ModelType, TranscodeType>
	// requestListener).
	// 监听资源加载的请求状态，可以使用两个回调：
	// onResourceReady(R resource, T model, Target<R> target, boolean
	// isFromMemoryCache, boolean isFirstResource)
	// 和onException(Exception e, T model, Target&lt;R&gt; target, boolean
	// isFirstResource)，
	...
	/**
	 * 使用Application上下文，Glide请求将不受Activity/Fragment生命周期控制 使用activity
	 * 会受到Activity生命周期控制 使用FragmentActivity 会受到FragmentActivity生命周期控制
	 * 
	 * @param context
	 * @param path
	 * @param imageView
	 ...
	 *            错误
	 * @param bitmapOrgif
	 *            加载普通图片 或者GIF图片 ，GIF图片设置bitmap显示第一帧
	 */
	...
	// ---------------------圆形图片-------------------
	/**
	 * 加载设置圆形图片 使用Application上下文，Glide请求将不受Activity/Fragment生命周期控制 <BR/>
	 * 使用activity 会受到Activity生命周期控制 <BR/>
	...
	// -----------------------圆角图片----------------------
	/**
	 * 加载设置圆角图片 使用Application上下文，Glide请求将不受Activity/Fragment生命周期控制 <BR/>
	 * 使用activity 会受到Activity生命周期控制 <BR/>
	 * 使用FragmentActivity 会受到FragmentActivity生命周期控制
	 * 
	 * @param context
	...
	// -------------------------------------------------
	/**
	 * Glide 加载模糊图片 使用Application上下文，Glide请求将不受Activity/Fragment生命周期控制 <BR/>
	 * 使用activity 会受到Activity生命周期控制 <BR/>
	 * 使用FragmentActivity 会受到FragmentActivity生命周期控制
	 * 
	 * @param context
	...
	// ---------------------------------------------------------
	/**
	 * 旋转图片 使用Application上下文，Glide请求将不受Activity/Fragment生命周期控制 <BR/>
	 * 使用activity 会受到Activity生命周期控制 <BR/>
	 * 使用FragmentActivity 会受到FragmentActivity生命周期控制
	 * 
	 * @param context
