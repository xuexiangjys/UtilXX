 ...
 * 使用4.0+系统上的HTTP缓存来代替磁盘缓存；只能得到结果，不能监听图片下载过程 <BR/>
 * Picasso 可以与okhttp搭配
 * 
 * 如果使用Picasso同时也使用了okhttp库，那么项目运行的时候可能会报出一下异常： Picasso detected an unsupported
 * OkHttp on the classpath 针对该情况，网上说需要引入：compile
 * 'com.squareup.okhttp:okhttp-urlconnection:2.2.0' 即同时引入一下三个包： compile
 * 'com.squareup.okhttp:okhttp:2.4.0' compile
 * 'com.squareup.okhttp:okhttp-urlconnection:2.2.0' compile
...
public class PicassoUtils {
	...
	// Picasso.with(context).load(url) .resize(50,
	// 50).centerCrop().into(imageView)
	// //这里的placeholder将resource传入通过getResource.getDrawable取资源，所以可以是张图片也可以是color
	// id
	...
	// //这里显示notification的图片
	// Picasso.with(activity).load(Data.URLS[new
	// Random().nextInt(Data.URLS.length)]).resizeDimen(R.dimen.notification_icon_width_height,
	// R.dimen.notification_icon_width_height).into(remoteViews, R.id.photo,
	 ...
	 *            //图片来源于Content provider String contentprividerUrl =
	 *            "content://media/external/audio/albumart/13"; <BR/>
	 *            //图片来源于assets // String assetsUrl =
	 *            Scheme.ASSETS.wrap("image.png"); <BR/>
	 *            String assetsUrl = "assets://fail_image.9.png"; <BR/>
	 *            //图片来源于 drawable // String drawableUrl =
	 *            Scheme.DRAWABLE.wrap("R.drawable.ic_launcher.png");<BR/>
	 *            String drawableUrl = "drawable://" + R.drawable.ic_add; <BR/>
	 *            //图片来源于 网络 String neturi =
	 *            "http://ww2.sinaimg.cn/large/49aaa343jw1dgwd0qvb4pj.jpg";<BR/>
	 *            <P>
	 * @param imageView
	 ...
	 *            PICASSO_BITMAP_SHOW_CIRCLE_TYPE ，
	 *            PICASSO_BITMAP_SHOW_ROUND_TYPE
	 *            ，PICASSO_BITMAP_SHOW_NORMAL_TYPE
	 * @param roundRadius
	 *            设置圆角半径
	 */
	 ...
	 *            设置圆角半径
	 */
	public void LoadImage(Context context, int localimage, int placeid, int errorid, ImageView imageView, String bitmapShowType, float roundRadius) {
		if (bitmapShowType.equals(PICASSO_BITMAP_SHOW_CIRCLE_TYPE)) {
			Picasso.with(context).load(localimage).placeholder(placeid).error(errorid).transform(new CircleTransform()).into(imageView);
		} else if (bitmapShowType.equals(PICASSO_BITMAP_SHOW_ROUND_TYPE)) {
