 ...
 * 数据表操作类
 */
public class AutoDBService<T> {
	private Context mContext;
	private Dao<T, Integer> mDao;
	private AutoDatabaseHelper<T> mDatabaseHelper;
	private DatabaseConnection mConnection;
	private Savepoint savePoint;

	public AutoDBService(Context context, Class<T> clazz) throws SQLException {
		mContext = context;
		mDatabaseHelper = new AutoDatabaseHelper<T>(mContext, clazz);
		if (mDao == null) {
			mDao = mDatabaseHelper.getDao(clazz);
		}
	}

	public AutoDatabaseHelper<T> getDatabaseHelper() {
