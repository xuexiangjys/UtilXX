package com.xuexiang.util.data.cachedisk;
...
public class DiskLruCacheHelper {
	...
	private DiskLruCache generateCache(Context context, File dir, int maxCount) throws IOException {
		...
		DiskLruCache diskLruCache = DiskLruCache.open(dir, appVersion, DEFAULT_VALUE_COUNT, maxCount);

		return diskLruCache;
	}

	private DiskLruCache generateCache(Context context, String dirName, int maxCount) throws IOException {
		DiskLruCache diskLruCache = DiskLruCache.open(getDiskCacheDir(context, dirName), Utils.getAppVersion(context), DEFAULT_VALUE_COUNT, maxCount);
		return diskLruCache;
	}
