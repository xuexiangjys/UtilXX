...
import com.xuexiang.util.system.AbLogUtil;
...
public class AbViewUtil {
	 ...
	 * @return the int
	 */
	public static int scale(Context context, int displayWidth, int displayHeight, float pxValue) {
		if(pxValue == 0 ){
			return 0;
		}
		float scale = 1;
		try {
			float scaleWidth = (float) displayWidth / DisplayUtils.getScreenW(context);
			float scaleHeight = (float) displayHeight /DisplayUtils.getScreenH(context);
