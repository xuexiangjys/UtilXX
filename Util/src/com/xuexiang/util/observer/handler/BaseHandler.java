...
public class BaseHandler extends Handler implements BaseHandlerMethod {
	...
	public void handleMessage(Message msg) {
		if (null != arrayReference && arrayReference.size() > 0) {
			if (null != baseHandlerGetKey) {
				SoftReference<BaseHandlerUpDate> reference = arrayReference.get(baseHandlerGetKey.handlerGetKey());
				if (null != reference) {
	...
	public void addSparseArray(BaseHandlerUpDate activity) {
		if (null != baseHandlerGetKey) {
			arrayReference.put(baseHandlerGetKey.handlerGetKey(), new SoftReference<BaseHandlerUpDate>(activity));
		} else
