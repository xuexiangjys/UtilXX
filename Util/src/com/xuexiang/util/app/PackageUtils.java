...
import com.xuexiang.util.common.ShellUtils.CommandResult;
...
public final class PackageUtils {
    ...
    /**
     * 使用系统自带的安装asset下的apk应用
     * @param context
     * @param appPackageName 应用的包名称
     * @param appName  应用名称
     * @param apkFileName apk文件的名称
     */
    public static void intallAPKFromAssetBySystem(final Context context, String appPackageName, String appName, final String apkFileName) {
        ...
        if(PackageUtils.checkAPP(context, appPackageName)) {
        	ToastUtil.getInstance(context).showToast("已安装" + appName + "!");
        	PackageUtils.openApp(context, appPackageName);
        } else {
        	ToastUtil.getInstance(context).showToast("未安装" + appName + "!");
        	if(ResourceUtils.copyAPKFromAssets(context, apkFileName)) {
        		DialogUtil.showChangePrompt(context, "是否安装" + appName + "应用？", new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						PackageUtils.installApk(context,LocalFileUtil.APK_PATH + apkFileName);
					}}, null);            		
        	} else {
        		ToastUtil.getInstance(context).showToast("获取APK资源失败！");
        	}
        }
	}
        		...
        		DialogUtil.showChangePrompt(context, "是否安装" + appName + "应用？", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						if (PackageUtils.install(context,LocalFileUtil.APK_PATH + apkFileName) == PackageUtils.INSTALL_SUCCEEDED) {
							ToastUtil.getInstance(context).showToast("安装" + appName + "成功！");
	            			PackageUtils.openApp(context, appPackageName);
	            		} else {
	            			ToastUtil.getInstance(context).showToast("安装" + appName + "失败！");
	            		}
					}}, null);            		
        	} else {
        		ToastUtil.getInstance(context).showToast("获取APK资源失败！");
