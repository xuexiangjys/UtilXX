...
public class LikeButton extends FrameLayout implements View.OnClickListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        LayoutInflater.from(getContext()).inflate(RUtils.getLayout(context, "likeview"), this, true);
        icon = (ImageView) findViewById(RUtils.getId(context, "icon"));
        dotsView = (DotsView) findViewById(RUtils.getId(context, "dots"));
        circleView = (CircleView) findViewById(RUtils.getId(context, "circle"));

        final TypedArray array = context.obtainStyledAttributes(attrs, RUtils.getStyleable(context, "LikeButton"), defStyle, 0);

        iconSize = array.getDimensionPixelSize(RUtils.getStyleableAttribute(context, "LikeButton_icon_size"), -1);
        if (iconSize == -1)
            iconSize = 40;

        String iconType = array.getString(RUtils.getStyleableAttribute(context, "LikeButton_icon_type"));

        likeDrawable = getDrawableFromResource(array, RUtils.getStyleableAttribute(context, "LikeButton_like_drawable"));

        if(likeDrawable!=null)
            setLikeDrawable(likeDrawable);

        unLikeDrawable = getDrawableFromResource(array, RUtils.getStyleableAttribute(context, "LikeButton_unlike_drawable"));
        ...
        circleStartColor = array.getColor(RUtils.getStyleableAttribute(context, "LikeButton_circle_start_color"), 0);

        if (circleStartColor != 0)
            circleView.setStartColor(circleStartColor);

        circleEndColor = array.getColor(RUtils.getStyleableAttribute(context, "LikeButton_circle_end_color"), 0);

        if (circleEndColor != 0)
            circleView.setEndColor(circleEndColor);

        dotPrimaryColor = array.getColor(RUtils.getStyleableAttribute(context, "LikeButton_dots_primary_color"), 0);
        dotSecondaryColor = array.getColor(RUtils.getStyleableAttribute(context, "LikeButton_dots_secondary_color"), 0);
        ...
        setEnabled(array.getBoolean(RUtils.getStyleableAttribute(context, "LikeButton_is_enabled"), true));
        Boolean status = array.getBoolean(RUtils.getStyleableAttribute(context, "LikeButton_liked"), false);
        setAnimationScaleFactor(array.getFloat(RUtils.getStyleableAttribute(context, "LikeButton_anim_scale_factor"), 3));
        setLiked(status);
    ...
    private Icon parseIconType(String iconType) {
        List<Icon> icons = Utils.getIcons(getContext());
    ...
    public void setExplodingDotColorsRes( int primaryColor,  int secondaryColor) {
        dotsView.setColors( getContext().getResources().getColor(primaryColor), getContext().getResources().getColor(secondaryColor));
    }
