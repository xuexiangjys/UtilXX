package com.xuexiang.view.satellitemenu;
...
public class SatelliteMenu extends FrameLayout {
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		LayoutInflater.from(context).inflate(RUtils.getLayout(context, "sat_main"), this, true);		
		imgMain = (ImageView) findViewById(RUtils.getId(context, "sat_main"));

		if(attrs != null){			
            TypedArray typedArray = context.obtainStyledAttributes(attrs, RUtils.getStyleable(context, "SatelliteMenu"), defStyle, 0);					
			satelliteDistance = typedArray.getDimensionPixelSize(MResource.getIdByName(context, "styleable", "SatelliteMenu_satelliteDistance"), DEFAULT_SATELLITE_DISTANCE);
			totalSpacingDegree = typedArray.getFloat(MResource.getIdByName(context, "styleable", "SatelliteMenu_totalSpacingDegree"), DEFAULT_TOTAL_SPACING_DEGREES);
			closeItemsOnClick = typedArray.getBoolean(MResource.getIdByName(context, "styleable", "SatelliteMenu_closeOnClick"), DEFAULT_CLOSE_ON_CLICK);
			expandDuration = typedArray.getInt(MResource.getIdByName(context, "styleable", "SatelliteMenu_expandDuration"), DEFAULT_EXPAND_DURATION);
	...
	public void addItems(List<SatelliteMenuItem> items) {
		...
		for (SatelliteMenuItem menuItem : menuItems) {
			...
			ImageView itemView = (ImageView) LayoutInflater.from(getContext())
					.inflate(RUtils.getLayout(getContext(), "sat_item_cr"), this, false);
