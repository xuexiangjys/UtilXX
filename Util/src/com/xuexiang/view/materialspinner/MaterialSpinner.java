...
package com.xuexiang.view.materialspinner;
...
public class MaterialSpinner extends TextView {
  ...
  private void init(Context context, AttributeSet attrs) {
    TypedArray ta = context.obtainStyledAttributes(attrs, RUtils.getStyleable(context, "MaterialSpinner"));
    int defaultColor = getTextColors().getDefaultColor();
    boolean rtl = Utils.isRtl(context);

    try {
      backgroundColor = ta.getColor(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_background_color"), Color.WHITE);
      textColor = ta.getColor(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_text_color"), defaultColor);
      arrowColor = ta.getColor(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_arrow_tint"), textColor);
      hideArrow = ta.getBoolean(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_hide_arrow"), false);
      popupWindowMaxHeight = ta.getDimensionPixelSize(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_dropdown_max_height"), 0);
      popupWindowHeight = ta.getLayoutDimension(RUtils.getStyleableAttribute(context, "MaterialSpinner_ms_dropdown_height"),
    ...
    left = right = bottom = top = resources.getDimensionPixelSize(RUtils.getDimen(context, "ms__padding_top"));
    if (rtl) {
      right = resources.getDimensionPixelSize(RUtils.getDimen(context, "ms__padding_left"));
    } else {
      left = resources.getDimensionPixelSize(RUtils.getDimen(context, "ms__padding_left"));
    }

    setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
    setClickable(true);
    setPadding(left, top, right, bottom);
    setBackgroundResource(RUtils.getDrawable(context, "ms__selector"));
    ...
    if (!hideArrow) {
      arrowDrawable = Utils.getDrawable(context, RUtils.getDrawable(context, "ms__arrow")).mutate();
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      popupWindow.setBackgroundDrawable(Utils.getDrawable(context, RUtils.getDrawable(context, "ms__drawable")));
    } else {
      popupWindow.setBackgroundDrawable(Utils.getDrawable(context, RUtils.getDrawable(context,"ms__drop_down_shadow")));
    }
  ...
  public <T> void setItems(@NonNull List<T> items) {
    ...
    adapter = new MaterialSpinnerAdapter<T>(getContext(), items).setTextColor(textColor);
...
//    popupWindow.showAtLocation(this, Gravity.TOP | Gravity.START, x, y);
  ...
  private int calculatePopupWindowHeight() {
    ...
    float listViewHeight = adapter.getCount() * getResources().getDimension(RUtils.getDimen(getContext(), "ms__item_height"));
