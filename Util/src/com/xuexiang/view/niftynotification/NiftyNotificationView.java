...
public class NiftyNotificationView {
	...
	private NiftyNotificationView(Activity activity, CharSequence text,
			Effects effects, ViewGroup viewGroup) {
	...
	private NiftyNotificationView(Activity activity, CharSequence text,
			Effects effects, ViewGroup viewGroup, Configuration configuration) {
	...
	public static NiftyNotificationView build(Activity activity,
			CharSequence text, Effects effects, int viewGroupResId) {
		return new NiftyNotificationView(activity, text, effects,
	...
	private TextView initializeTextView() {
		...
		text.setBackgroundColor(Color
				.parseColor(this.configuration.backgroundColor));

		if ((null != iconDrawable) || (0 != iconRes)) {
			text.setMinHeight(viewHeight);
			text.setGravity(isDefault ? Gravity.CENTER_VERTICAL
					: this.configuration.textGravity);
		} else {
			text.setGravity(isDefault ? Gravity.CENTER
					: this.configuration.textGravity);
		}
		return text;
	}
	...
	private ImageView initializeImageView() {
		...
		image.setBackgroundColor(Color
				.parseColor(this.configuration.iconBackgroundColor));
	...
	public NiftyNotificationView setOnClickListener(
