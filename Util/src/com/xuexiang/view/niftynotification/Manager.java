...
public class Manager extends Handler {
	...
	void addSticky(NiftyNotificationView crouton) {
		Log.i("count", crouton.getViewGroup().getChildCount() + "   "
				+ notifyQueue.size());

		if (crouton.getViewGroup().getChildCount() == 0
				&& notifyQueue.size() == 1) {
			removeSticky();
		}
	...
	private void sendMessageDelayed(NiftyNotificationView crouton,
			...
			observer.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
				...
				public void onGlobalLayout() {
					if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
						notifyView.getViewTreeObserver()
					...
					notify.getEffects()
							.getAnimator()
							.setDuration(notify.getConfiguration().animDuration)
							.in(notify.getView());
					if (!isSticky) {
						sendMessageDelayed(
								notify,
	...
	protected void removeNotify(NiftyNotificationView notify) {
		...
		if (null != notifyParentView) {
			notify.getEffects().getAnimator()
					.setDuration(notify.getConfiguration().animDuration)
					.out(notify.getView());
			sendMessageDelayed(notify, Messages.REMOVE_NOTIFICATION_VIEW,
					notify.getOutDuration());

			sendMessageDelayed(notify, Messages.DISPLAY_NOTIFICATION,
					notify.getOutDuration());
		}
	}
