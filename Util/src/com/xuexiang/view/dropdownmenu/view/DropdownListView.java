...
public class DropdownListView extends ScrollView {
    ...
    private void init() {
        View view =  LayoutInflater.from(getContext()).inflate(RUtils.getLayout(getContext(), "dropdown_tab_list"), this,true);
        linearLayout = (LinearLayout) view.findViewById(RUtils.getId(getContext(), "linearLayout"));
    }
    ...
    public void bind(List<? extends DropdownItemObject> list,
        ...
        for (DropdownItemObject item : list) {
            if (isFirst) {
                ...
                if (divider == null) {
                    divider = inflater.inflate(RUtils.getLayout(getContext(), "dropdown_tab_list_divider"), linearLayout, false);
                }
                linearLayout.addView(divider);
            }
            DropdownListItemView view = cachedViews.poll();
            if (view == null) {
                view = (DropdownListItemView) inflater.inflate(RUtils.getLayout(getContext(), "dropdown_tab_list_item"), linearLayout, false);
            }
            view.setTag(item);
