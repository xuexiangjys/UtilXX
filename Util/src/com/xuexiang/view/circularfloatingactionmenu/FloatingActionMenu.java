 ...
 */
package com.xuexiang.view.circularfloatingactionmenu;
...
import com.xuexiang.view.circularfloatingactionmenu.animation.DefaultAnimationHandler;
import com.xuexiang.view.circularfloatingactionmenu.animation.MenuAnimationHandler;
...
public class FloatingActionMenu {
    ...
    private int radius;
    /** List of menu items */
    private List<Item> mSubActionItems;
    ...
    public FloatingActionMenu(final View mainActionView,
        ...
        this.radius = radius;
        mSubActionItems = subActionItems;
        ...
        for (int i = 0; i < mSubActionItems.size(); i++) {
            	...
            	item.view.setOnClickListener(new OnClickListener(){
        			...
        			public void onClick(View v) {
        				if (mMenuItemOnClickListener != null) {
        					mMenuItemOnClickListener.onMenuItemClick(item.view, item.position);
        					toggle();
        				}
        			}
        		});
    ...
    public void open(boolean animated) {
        ...
        if(animated && animationHandler != null) {
            ...
            for (int i = 0; i < mSubActionItems.size(); i++) {
                ...
                if (mSubActionItems.get(i).view.getParent() != null) {
                ...
                final FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(mSubActionItems.get(i).width, mSubActionItems.get(i).height, Gravity.TOP | Gravity.LEFT);

                if(systemOverlay) {
                    params.setMargins(center.x - overlayParams.x - mSubActionItems.get(i).width / 2, center.y - overlayParams.y - mSubActionItems.get(i).height / 2, 0, 0);
                }
                else {
                    params.setMargins(center.x - mSubActionItems.get(i).width / 2, center.y - mSubActionItems.get(i).height / 2, 0, 0);
                }
                addViewToCurrentContainer(mSubActionItems.get(i).view, params);
            }
            ...
            for (int i = 0; i < mSubActionItems.size(); i++) {
                ...
                final FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(mSubActionItems.get(i).width, mSubActionItems.get(i).height, Gravity.TOP | Gravity.LEFT);
                if(systemOverlay) {
                    params.setMargins(mSubActionItems.get(i).x - overlayParams.x, mSubActionItems.get(i).y - overlayParams.y, 0, 0);
                    mSubActionItems.get(i).view.setLayoutParams(params);
                }
                else {
                    params.setMargins(mSubActionItems.get(i).x, mSubActionItems.get(i).y, 0, 0);
                    mSubActionItems.get(i).view.setLayoutParams(params);
    ...
    public void close(boolean animated) {
        ...
        if(animated && animationHandler != null) {
            ...
            for (int i = 0; i < mSubActionItems.size(); i++) {
                removeViewFromCurrentContainer(mSubActionItems.get(i).view);
            }
    ...
    private Point calculateItemPositions() {
        ...
        int divisor;
        if(Math.abs(endAngle - startAngle) >= 360 || mSubActionItems.size() <= 1) {
            divisor = mSubActionItems.size();
        }
        else {
            divisor = mSubActionItems.size() -1;
        }
        ...
        for(int i=0; i<mSubActionItems.size(); i++) {
            ...
            mSubActionItems.get(i).x = (int) coords[0] - mSubActionItems.get(i).width / 2;
            mSubActionItems.get(i).y = (int) coords[1] - mSubActionItems.get(i).height / 2;
        }
        return center;
    }
    ...
    private WindowManager.LayoutParams calculateOverlayContainerParams() {
        ...
        for(int i=0; i < mSubActionItems.size(); i++) {
            int lm = mSubActionItems.get(i).x;
            int tm = mSubActionItems.get(i).y;
            ...
            if(lm + mSubActionItems.get(i).width > right) {
                right = lm + mSubActionItems.get(i).width;
            }
            if(tm + mSubActionItems.get(i).height > bottom) {
                bottom = tm + mSubActionItems.get(i).height;
            }
        }
     ...
     * A listener to listen open/closed state changes of the Menu
     */
    public static interface MenuItemOnClickListener {
    ...
    public static class Builder {
        ...
        public Builder(Context context, boolean systemOverlay) {
            ...
            // Default settings
            radius = context.getResources().getDimensionPixelSize(RUtils.getDimen(context, "action_menu_radius"));
            startAngle = 180;
            endAngle = 270;
        ...
        public Builder addSubActionView(View subActionView, int width, int height) {
            subActionItems.add(new Item(subActionView, position++,width, height));
            return this;
        }
        ...
        public Builder setMenuItemOnClickListener(MenuItemOnClickListener listener) {
        	menuItemOnClickListener = listener;
            return this;
        }
